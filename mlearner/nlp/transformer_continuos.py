...
class Encoder(layers.Layer):
    ...
    def call(self, inputs, mask, training):
        ...
        outputs = inputs  # [:, tf.newaxis, tf.newaxis, :]
...
class Transformer_continuos(tf.keras.Model):
    """
    Transformer
    ...
    def __init__(self,
                 ...
                 dropout_rate,
                 name="transformer"):
        super(Transformer_continuos, self).__init__(name=name)
    ...
    def create_padding_mask(self, seq):  # seq: (batch_size, seq_length)
        mask = tf.cast(tf.math.equal(seq[:, :, 1], 0), tf.float32)
    ...
    def call(self, enc_inputs, dec_inputs, training):
        ...
        # )
        dec_mask_1 = self.create_padding_mask(dec_inputs)
...
def Transformer_continuos_train(Transformer,
                        dataset,
                        d_model,
                        epochs,
                        train=True,
    ...
    def train_step(enc_inputs, dec_inputs, dec_outputs_real):
        with tf.GradientTape() as tape:
            ...
            # loss_array = [loss_function(dec_outputs_real[:, :, i], 
            #                 predictions[:, :, i]) for i in range(dec_outputs_real.shape[2])]
