...
data_out = pd.DataFrame({"a": [2.0, 2.0, 2.0, 3.0, 4.0, 5.0], "b": ["OK", 2, 2, 2, 4, 5]})
col = ["a"]
col_test = ["a", "b"]
col_no_include = ["a", "c"]
...
def test_fitting_error():
    ...
    ft = FillNaTransformer_mean(columns=col)
...
def test_col_fail_type():
    with pytest.raises(NameError):
        FillNaTransformer_mean(columns=np.array(col_test))


def test_col_object():
    ft = FillNaTransformer_mean(columns=col_test)
    ft.fit(data)
...
def test_col_no_include():
    ft = FillNaTransformer_mean(columns=col_no_include)
    with pytest.raises(NameError):
        ft.fit(data)
