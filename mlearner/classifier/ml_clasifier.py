...
from mlerner.utils import ParamsManager
...
class TrainingUtilities(object):
    ...
    def Seleccion_rasgos(self, model, X , Y, n, display=True):
        """
        Función que nos retorna las categorias que más influyen en el modelo.

        Inputs:
            - X, Y: dataset.
            - n: número de categorias que queremos que formen parte de variables del modelo de RL.
    ...
    def optimizacion_seleccion_rasgos(self, model, X, Y, n_splits, display=True):
        """
        Selección del número de categorias a tener en cuenta
        """
        scores = []
        nums = []
    ...
    def opt_RandomForest_Classifier(self, X, Y, nmin=1, nmax=10000, num=4, display=True):
        """
        Selección del número de categorias a tener en cuenta para RF
        """
        ...
        for i in list(rango):
            ...
            forest, forest.oob_score_ = model_RandomForestClassifier(X_train, y_train, int(i))
...
class PipelineClasificators(Training):
    ...
    def __init__(self, random_state=99):
        """
        Inicialización de la clase de modelos
        """
        self.random_state = random_state
        self.manager_models = ParamsManager(param_file, key_read="Models")
    ...
    def RandomForestClassifier(self):
        """
        n_jobs: Parelización en la computación.
    ...
    def XGBoost(self, name = "CBT"):
        ...
        """
        params = self.manager_models.get_params()["XGBoost"]
    ...
    def Pipeline_SelectModel(self, X, y, n_splits=5, select="XGBoost"):
        ...
        if self.manager_models.get_params()["select_models"]["XGBoost"]:
            ...
            train_start = time.perf_counter()
            score, results_XGBoost = _model.fit_cv(X, y, nfold=n_splits, verbose=0)
            self.scores.append(score)
            train_end = time.perf_counter()
        ...
        if self.manager_models.get_params()["select_models"]["LightGBM"]:
            ...
            _model.fit(X, y, verbose=0, mute=True)
            train_start = time.perf_counter()
            score, results_LightGBM = _model.fit_cv(X, y, nfold=n_splits, verbose=0)
            self.scores.append(score)
            train_end = time.perf_counter()
    ...
    def pred_binary(self, prob_pred, y_target):
        return accuracy_score(y_target, np.where(preds > 0.5, 1, 0))
